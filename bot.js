...
bot.on('ready', () => {
 ...
 bot.on('ready', () => {
   bot.user.setActivity("Eve Support")
...
bot.on('message', msg => {
  ...
  if (talkedRecently.has(msg.author.id)) {
      msg.channel.send("** الرجاء الانتظار دقيقتين لفتح تذكرة اخرى ** " + msg.author)
    .then(msg => {
    msg.delete(10000)
  })
  ...
  if (setTickets == false) {
      msg.channel.send("  ** `set` هذه ليست روم لفتح التذاكر على الاونر ان يحدد روم التذكرة بالامر  ** ")
      return;
    }
  ...
  if (!args[1]) {
      msg.channel.send( "** الرجاء كتابة سبب فتح التذكرة بعد الامر ** " )
      return;
    }
  ...
  //  cChannel.send(embedNewLog)
    ...
    if (!msg.member.hasPermission('MANAGE_MESSAGES')) {
      msg.channel.send('** ليست لديك الصلاحيات لفعل هذا **')
      return;
    }
    setTickets = true

    ticketsChannel = msg.channel
    msg.channel.send("** اصبحت هذه القناة مخصصة للتذاكر **")
    ...
    if (!msg.member.hasPermission('MANAGE_MESSAGES')) {
      ...
      if (tickets.length < 1) {
        msg.channel.send("** ليست هناك اي تذاكر للانتظار **")
        return;
      }
    ...
    var embedHelp = new Discord.RichEmbed()
       ...
       break;
        ...
        case "clearall":
         ...
         msg.channel.send('** هذا اللوق خاص بالتذاكر الان **')
         break;
  }
})
...
bot.on('channelCreate', channel => {
  ...
  channel.overwritePermissions(channel.guild.roles.find(role => role.name === "Helper"), {
...
channel.send(`<@${(tickets[tickets.length-1]).author.id}>** سيأتي فريق العمل قريبا لمساعدتك الرجاء كتابة ماذا تريد في التذكرة اثناء الانتظار ** :`)
...
bot.on('message', message => {
 ...
 bot.user.setActivity(argresult, {type:'WATCHING'});
  ...
  if (message.content.startsWith(adminprefix + 'sets')) {
   bot.user.setGame(argresult, "https://www.twitch.tv/dream");
